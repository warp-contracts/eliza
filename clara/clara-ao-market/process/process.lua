...
function AGENTS_MARKET.v1.RegisterAgentProfile(msg)
    ...
    for i in ipairs(AGENTS_MARKET.Storage.Agents) do
        ...
        if (AGENTS_MARKET.Storage.Agents[i].id == agentId) then
...
function AGENTS_MARKET.v1.RegisterTask(msg)
    ...
    -- agentId required only for topic "chat"
    local agentId = msg.Tags["RedStone-Agent-Id"]
...
function _assertAgentsRegistered()
    assert(#AGENTS_MARKET.Storage.Agents > 0, "No agents registered")
end
...
function _assertAgentRegistered(agentId, sender)
    ...
    assert(utils.find(function(x)
        ...
        "Agent " .. agentId .. " not registered for sender " .. sender .. ". Required for topic = chat")
end
...
function _filterAgentsWithTopicAndFeeAndNotDeclinedTask(topic, taskId, reward, requesterId)
    ...
    return utils.filter(function(x)
        return tonumber(x.fee) <= tonumber(reward)
            and x.topic == topic
            and (x.tasks.declined == nil or not utils.includes(taskId)(x.tasks.declined)
            and (requesterId == nil or (requesterId ~= x.id)))
    end)(AGENTS_MARKET.Storage.Agents)
end
...
function _leastOccupiedStrategy(topic, taskId, reward, requesterId)
    ...
    -- assert(false, "inside _leastOccupiedStrategy " .. json.encode(agentsWithTopic))
    local currentLowest
    local agentIdx
    for i, v in ipairs(agentsWithTopic) do
        if (currentLowest == nil) then
            currentLowest = #v.tasks.inbox
            agentIdx = i
            goto continue -- omfg
        end
        if (#v.tasks.inbox < currentLowest) then
            currentLowest = #v.tasks.inbox
            agentIdx = i
        end
        :: continue ::
